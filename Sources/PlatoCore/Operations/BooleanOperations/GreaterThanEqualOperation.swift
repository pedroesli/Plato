//
//  GreaterThanEqualOperation.swift
//
//
//  Created by Pedro Ésli Vieira do Nascimento on 03/02/24.
//

struct GreaterThanEqualOperation: BaseOperation {
    let left: Value
    let right: Value
    let order: OrderType
    ...
    func result() throws -> Value {
        ...
        default:
            fatalError("Greater that equal operation failed. Reason: unsupported operation for \(order.high) type")
        }
    }
}
