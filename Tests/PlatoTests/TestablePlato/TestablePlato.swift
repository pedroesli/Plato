...
class TestablePlato {
    ...
    init(configuration: PlatoConfiguration = PlatoConfiguration()) {
        self.interpreter = TestablePlatoInterpreter(configuration: configuration)
        self.plato = Plato(platoInterpreter: self.interpreter)
    }
